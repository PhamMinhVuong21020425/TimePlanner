            ...
            return (
                <div className="bg-emerald-50 rounded-md p-4">
                    <div className="text-xs text-center w-3/5 rounded-md">
                        <div className="bg-emerald-300 px-2 py-1 font-bold rounded-md text-emerald-600 text-xs -translate-y-6">
                            {i.type.replace(/_/g, " ")}
                        </div>
                    </div>
        ...
        case 'MEDIUM':
            return (
                <div className=" bg-amber-50 rounded-md p-4">
                    <div className="text-xs text-center w-3/5 rounded-md">
                        <div className="bg-amber-300 px-2 py-1 font-bold rounded-md text-amber-600 text-xs -translate-y-6">
                            {i.type.replace(/_/g, " ")}
                        </div>
                    </div>
                    <div key={id} className="font-bold text-base text-gray-600 -translate-y-3 break-words">
        ...
        case 'HIGH':
            return (
                <div className="bg-rose-50 rounded-md p-4">
                    <div className="text-xs text-center w-3/5 rounded-md">
                        <div className="bg-rose-300 px-2 py-1 font-bold rounded-md text-rose-600 text-xs -translate-y-6">
                            {i.type.replace(/_/g, " ")}
                        </div>
                    </div>
...
function buttonStyle(i: Task) {
    return (
        <div>
            {i.status === 'STOPPED' ? (
                <button className="text-xs text-rose-600 bg-rose-200 font-bold px-2 py-1 rounded-md">
                    Stopped
                </button>
            ) : i.status === 'INPROGRESS' ? (
                <button className="text-xs text-cyan-600 bg-cyan-200 font-bold px-2 py-1 rounded-md">
                    In Progress
                </button>
            ) : i.status === 'COMPLETED' ? (
                <button className="text-xs text-green-600 bg-green-200 font-bold px-2 py-1 rounded-md">
                    Completed
                </button>
            ) : null}
        </div>
